...
const posturl = async function (req, res) {
    ...
    try {
        ...
        data = { longUrl: longUrl, shortUrl: shortUrl, urlCode: short }

        let urlData = await urlModel.create(data)


        await SET_ASYNC(short, longUrl)

        await SET_ASYNC(longUrl, JSON.stringify(urlData))  //to set expiry of cache (,"EX",60*60)
